...
const getHtmlForWebview = (options) => {
  const { webview, extensionUri, labels, loading, currentPage, dpmm, width, height, units, dataText } = options;

  const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'modules', 'index.js'));
  const scriptElementsUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'modules', 'bundled.js'));

  const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'modules', 'index.css'));
  const styleHighliteJsUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'modules', 'atom-one-dark.css'));
  ...
  <body>
    ...
    <vscode-form-group>
      ...
      <vscode-single-select id="label_size__units">
        <vscode-option ${units === 'inches' && 'selected'} value="inches">inches</vscode-option>
        <vscode-option ${units === 'cm' && 'selected'} value="cm">cm</vscode-option>
        <vscode-option ${units === 'mm' && 'selected'} value="mm">mm</vscode-option>
      </vscode-single-select>
