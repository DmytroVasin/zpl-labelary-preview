...
class ZplPreviewer {
  constructor(context) {
    ...
    this.units = this.context.globalState.get('units', 'inches')
    ...
    this.panel.webview.onDidReceiveMessage((message) => {
      switch (message.type) {
        ...
        case 'refreshLabelSize':
          ...
          this.units = message.value.units
          this.context.globalState.update('units', this.units)
  ...
  async fetchLabelaryLabel(page) {
    const url = buildLabelaryUrl(this.dpmm, this.width, this.height, this.units, page)

    if (this.cache[url]) {
      return this.cache[url]
    }

    const response = await makeLabelaryRequest(url, this.dataText, 'png')
